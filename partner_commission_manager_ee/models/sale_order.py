...
class SaleOrder(models.Model):
    ...
    # Método para heredar MANAGER dependiendo del COMISIONISTA (referrer):
    ...
    @api.depends(
        "commission_plan_frozen",
        "partner_id",
        "referrer_id",
        "referrer_id.commission_plan_id",
        "manager_id",
    )
    def _compute_manager_commission_plan(self):
        for so in self:
            if not so.is_subscription and so.state in ["draft", "sent"]:
                so.manager_commission_plan_id = (
                    so.referrer_id.manager_commission_plan_id
                )
            ...
            values.update(
                {
                    ...
                    "commission_plan_frozen": self.referrer_id.commission_plan_id
                    != self.commission_plan_id,
                    "manager_id": self.manager_id.id,
