...
#    manager_id = fields.Many2one('res.partner', 'Referrer Manager', domain=[('grade_id', '!=', False)])
    ...
    def _get_commission_manager_purchase_order_domain(self):
        self.ensure_one()
    ...
    def _get_commission_manager_purchase_order(self):
        self.ensure_one()
        purchase = self.env['purchase.order'].sudo().search(
            self._get_commission_manager_purchase_order_domain(), limit=1)

        if not purchase:
            sales_rep = self._get_sales_representative()
            purchase = self.env['purchase.order'].with_context(
                mail_create_nosubscribe=True).sudo().create({
                'partner_id': self.manager_id.id,
                'currency_id': self.currency_id.id,
                'company_id': self.company_id.id,
                'fiscal_position_id': self.env['account.fiscal.position'].with_company(
                    self.company_id)._get_fiscal_position(self.manager_id).id,
                'payment_term_id': self.manager_id.with_company(
                    self.company_id).property_supplier_payment_term_id.id,
                'user_id': sales_rep and sales_rep.id or False,
                'dest_address_id': self.manager_id.id,
                'origin': self.name,
                'purchase_type': 'commission',
            })
    ...
    def _make_commission_manager(self):
        for move in self.filtered(
            ...
            for line in move.invoice_line_ids:
                rule = line._get_commission_manager_rule()
                if rule:
                    if not product:
                    ...
                    if not order:
                        ...
                        desc_lines += _("\n%s: from %s to %s", line.product_id.name,
                                        format_date(self.env,
                                                    line.subscription_start_date),
                                        format_date(self.env,
                                                    line.subscription_end_date))
            ...
            purchase = move._get_commission_manager_purchase_order()
            ...
            if move.move_type in ['out_invoice', 'in_invoice']:
                ...
                move.commission_manager_po_line_id = line
    ...
    def _reverse_moves(self, default_values_list=None, cancel=False):
        ...
        for move, default_values in zip(self, default_values_list):
            default_values.update({
                ...
                'manager_id': move.manager_id.id,
                'commission_manager_po_line_id': move.commission_manager_po_line_id.id,
            })
    ...
    def _invoice_paid_hook(self):
        ...
        self.filtered(lambda move: move.move_type == 'out_refund')._make_commission_manager()
        self.filtered(lambda move: move.move_type == 'out_invoice')._make_commission_manager()
        return res

    def button_draft(self):
        res = super(AccountMove, self).button_draft()
        for move in self:
            cpom = self.env['purchase.order'].sudo().search(
                move._get_commission_manager_purchase_order_domain(), limit=1)
            if cpom and move.move_type == 'out_refund' and cpom.state == 'draft':
                message_body = _(
                    "The commission partner order %s must be checked manually (especially refund lines which can be duplicated).",
                    cpo._get_html_link())
