        ...
        for record in self:
            if record.is_pair and record.product_tmpl_set_id.id:
                ...
                for pp in record.product_variant_ids:
                    ...
                    self.env['product.supplierinfo'].create({
                        'product_tmpl_id': record.id, 'product_id': pp.id, 'price': standard_price,
                        'currency_id': ptassortment.exwork_currency_id.id, 'partner_id': ptassortment.manufacturer_id.id})
                for pp in ptassortment.product_variant_ids:
                    price = standard_price * pp.pairs_count
                    pp.write({"standard_price": price})
                    self.env['product.supplierinfo'].create({
                        'product_tmpl_id': ptassortment.id, 'product_id': pp.id, 'price': price,
                        'currency_id': pp.exwork_currency_id.id, 'partner_id': ptassortment.manufacturer_id.id})
            ...
            if record.is_assortment and record.product_tmpl_single_id.id:
                ...
                for pp in record.product_variant_ids:
                    ...
                    pp.write({"standard_price": price})
                    self.env['product.supplierinfo'].create({
                        'product_tmpl_id': record.id, 'product_id': pp.id, 'price': price,
                        'currency_id': pp.exwork_currency_id.id, 'partner_id': record.manufacturer_id.id})
                for pp in ptsingle.product_variant_ids:
                    pp.write({"standard_price": standard_price})
                    self.env['product.supplierinfo'].create({
                        'product_tmpl_id':ptsingle.id, 'product_id':pp.id, 'price':standard_price,
                        'currency_id':pp.exwork_currency_id.id, 'partner_id':record.manufacturer_id.id})
